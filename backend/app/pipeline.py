...
def process_frame(raw_bytes: bytes):
    ...
    # Fallback: se nenhum contorno encontrado, tenta a imagem inteira como carta
    if not contours:
        logger.info(f"[{frame_id}] Fallback: processando imagem inteira como carta.")
        h, w = img.shape[:2]
        whole = np.array([[[0, 0]], [[w - 1, 0]], [[w - 1, h - 1]], [[0, h - 1]]], dtype=np.int32)
        contours = [whole]
